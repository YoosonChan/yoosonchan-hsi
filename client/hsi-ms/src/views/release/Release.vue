<template>
  <div id="h-container">
    ...
    <div class="h-operator">
      ...
      <a-button v-if="!(role === 'admin')" @click="visibleParams.add = true" type="primary" class="h-operator-btn">新增</a-button>
      <a-popconfirm
        title="确定删除选中？"
    ...
    <div class="h-table">
      ...
      >
        <span slot="type" slot-scope="type">
          <a-tag v-if="type === 0" color="blue">公告</a-tag>
          <a-tag v-if="type === 1" color="green">作业</a-tag>
          <a-tag v-if="type === 2" color="purple">统计</a-tag>
        </span>
        <span slot="content" slot-scope="content">
          <a-tooltip placement="top">
            ...
            <span style="cursor: pointer;">{{content.substring(0,20) + (content.length < 20 ? '' : '...') }}</span>
          </a-tooltip>
        </span>
        <span slot="operator" slot-scope="text, record">
          <a-button-group>
            <a-button v-if="!(role === 'admin')" @click="handleClickEdit(record)" size="small" type="primary">编辑</a-button>
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      localStorage.setItem('teacherId', '0012')
    },
    methods: {
      ...request,
      // 表格选择行操作
      ...
      // 添加数据
      handleAdd (value) {
        this.upsert({ ...value, teacherId: localStorage.getItem('teacherId') }).then(res => {
          this.refresh()
          this.visibleParams.add = false
        }).catch(err => {
          this.refresh()
        })
      },
